...
export type ProcessTask = {
  ...
  projectId?: string;   // Dynamic project ID
  buffer: Buffer;
  filename: string;
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          const projectId = task.projectId || '5e19dae4-46fe-42ba-981e-bd0a3a451f8e';
          const apiResponse = await sendFileToExternalApi(task.buffer, task.filename, task.webhookUrl, projectId);
