...
import { buildProceduresRows, createExportResponse, fetchJobs } from '../utils';
...
export async function GET(
  ...
  try {
    ...
    const rows = buildProceduresRows(jobs);

    const stateStr = stateParam ? `_${stateParam}` : '';
    const cityStr = cityParam ? `_${cityParam}` : '';
    const base = `Hospital_Procedures${stateStr}${cityStr}` || 'Hospital_Procedures';
    const filenameBase = `${base}_${new Date().toISOString().split('T')[0]}`;

    return createExportResponse(rows, filenameBase, format);
  } catch (error) {
    console.error('Error generating procedures export:', error);
    const message = error instanceof Error ? error.message : 'Unknown error';
    return new NextResponse(
      JSON.stringify({ error: 'Failed to generate procedures export', message }),
      {
        status: 500,
