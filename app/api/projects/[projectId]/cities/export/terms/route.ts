...
import { buildTermsRows, createExportResponse, fetchJobs } from '../utils';
...
export async function GET(
  ...
  try {
    ...
    const rows = buildTermsRows(jobs);

    const stateStr = stateParam ? `_${stateParam}` : '';
    const cityStr = cityParam ? `_${cityParam}` : '';
    const base = `Hospital_TermsAndNotes${stateStr}${cityStr}` || 'Hospital_TermsAndNotes';
    const filenameBase = `${base}_${new Date().toISOString().split('T')[0]}`;

    return createExportResponse(rows, filenameBase, format);
  } catch (error) {
    console.error('Error generating terms export:', error);
    const message = error instanceof Error ? error.message : 'Unknown error';
    return new NextResponse(
      JSON.stringify({ error: 'Failed to generate terms export', message }),
      {
        status: 500,
